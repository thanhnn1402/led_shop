<?php 
    ...
    $service_update = (isset($_GET["u"]) ? $_GET["u"] : '');
    ...
    if(!empty($service_update)) {
        ...
        $service = db_service_get_by_id($service_update);

        // echo $service['name'];
        // echo $service['description'];
        // die();

        if(is_submit("edit-service")) {
            ...
            if(!($error)) {
                ...
                $error = db_service_update($data, $service["id"]); 
        ...
        <div class="content-wrapper">
          <div class="container-fluid overflow-scroll bg-secondary bg-opacity-10" style="min-height: calc(100vh - 55px)">
            ...
            <div class="row bg-white my-3 mx-1 py-3 px-2 rounded-2">
              <div class="col-md-12">
              <form id="form-edit-service" class="form-login" method="post" action="<?php echo base_url('admin/?m=service&a=edit&u='.$service['name']); ?>" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <div class="form-group-flex">
                            <input type="file" class="form-control border-0" id="thumbnail" name="thumbnail" value="<?=isset($service["thumbnail"]) ? $service["thumbnail"] : '';?>">
                        </div>
                        <div class="row mt-4">
                            <div class="col-md-2 img-grid">
                                <img src="<?= isset($service["thumbnail"]) ? './storage/uploads/' . $service["thumbnail"] : ''?>" alt="" class="w-100">
                            </div>
                        </div>
                        <span class="form-message"></span>
                    </div>
                    <input type="hidden" name="request_name" value="edit-service"/>
